...
const scene = new Scene(Scenes.TRAINING15_MEDIUM);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING15_MEDIUM}.` });
    await ctx.reply(
        strings.trainings.training15.medium,
        keyboard([
            [
                { text: strings.menu },
