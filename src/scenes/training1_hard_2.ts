...
const scene = new Scene(Scenes.TRAINING1_HARD_2);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING1_HARD_2}.` });
    await ctx.reply(strings.trainings.training1.hard.package2, keyboard([[{ text: strings.menu }]]));
