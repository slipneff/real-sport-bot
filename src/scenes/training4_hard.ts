...
const scene = new Scene(Scenes.TRAINING4_HARD);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING4_HARD}.` });
    await ctx.reply(
        strings.trainings.training4.hard.greeting,
        keyboard([
            [
                { text: strings.menu },
