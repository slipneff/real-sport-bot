...
const scene = new Scene(Scenes.TRAINING11_MEDIUM);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING11_MEDIUM}.` });
    await ctx.reply(
        strings.trainings.training11.medium,
        keyboard([
            [
                { text: strings.menu },
