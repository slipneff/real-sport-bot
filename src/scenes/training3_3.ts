...
const scene = new Scene(Scenes.TRAINING3_3);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING3_3}.` });
    await ctx.reply(
        strings.trainings.training3.easy.package3,
        keyboard([
            [
                { text: strings.menu },
            ],
    ...
    await ctx.replyWithAnimation({ source: __dirname + '/../images/train3/1_3.MP4' });
});
