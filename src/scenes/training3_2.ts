...
const scene = new Scene(Scenes.TRAINING3_2);

scene.enter(async ctx => {
    signale.info({ prefix: ctx.chat.id, message: `ENTER ${Scenes.TRAINING3_2}.` });
    await ctx.reply(
        strings.trainings.training3.easy.package2,
        keyboard([
            [
                { text: strings.menu },
            ],
    ...
    await ctx.replyWithAnimation({ source: __dirname + '/../images/train3/1_2.MP4' });
});
